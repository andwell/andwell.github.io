...
DOWNLOAD_URI=http://devguo.com/tools/WubaTool.tar.gz
...
BUNDLE_ID="com.58.WubaTool"
TMP_FILE="$(mktemp -t ${BUNDLE_ID})"

# Remove WubaTool from Xcode's skipped plugins list if needed
if defaults read com.apple.dt.Xcode "$PLIST_PLUGINS_KEY" &> "$TMP_FILE"; then
# We read the prefs successfully, delete WubaTool from the skipped list if needed
...
'Please close Xcode before installing WubaTool.'
exit 1
}
defaults write com.apple.dt.Xcode "$PLIST_PLUGINS_KEY" "$(cat "$TMP_FILE")"
echo 'WubaTool was removed from Xcode'\''s skipped plugins list.' \
...
# Download and install WubaTool
mkdir -p "${PLUGINS_DIR}"
curl -L $DOWNLOAD_URI | tar xvz -C "${PLUGINS_DIR}"

beer='🏃🏃🏃🏃🏃🏃🏃🏃'

echo "WubaTool installed successfully $beer " \
